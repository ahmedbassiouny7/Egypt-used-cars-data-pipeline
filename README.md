...
If your shell does not expand paths inside Docker, use:

```bash
docker run --rm egypt-used-cars-airflow:local bash -c "python -m py_compile /opt/airflow/src/*.py /opt/airflow/dags/*.py"
```
